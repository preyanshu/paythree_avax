...
export function AppContent({ children }: AppContentProps) {
  ...
  useEffect(() => {
    const verifyEmployee = async () => {
      if (isConnected && address) {
        ...
        if (!employee) {
          setIsEmployee(false);
          setError('This wallet is not registered as an employee.');
        } else {
          setIsEmployee(true);
        }
  ...
  if (!isConnected || checkingEmployee || isEmployee === false) {
    return (
        ...
        <Card className="w-full max-w-md border-border bg-card/50 backdrop-blur-sm">
          <CardHeader className="text-center space-y-4">
            ...
            <CardDescription className="text-muted-foreground text-base">
              Connect your Ethereum wallet to access the Employee Portal
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            {checkingEmployee && (
              <div className="flex items-center gap-2 p-3 rounded-md bg-muted/10 border border-border animate-pulse">
                <span className="text-sm text-muted-foreground">Verifying employee status...</span>
              </div>
            )}
